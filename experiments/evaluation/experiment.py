...
class Experiment:
    def __init__(self, csv_file, num_test_data=5000, num_h_data=200):
    ...
    def performance_different_data_size(self, data_size_arr, quantile=0.95, use_sampling=True):
        ...
        for data_size in data_size_arr:
            ...
            for i in range(data_size, 5000 + data_size):
                current_data = self.dataset.iloc[i]
                m, s, r, l = current_data[data_col_by_keys(['m', 's', 'r', 'l'])]
                ...
                # result_gaussian.add(quantile, l, estimated_latency, (time.time() - start) * 1000, m, ft)
                #
                # start = time.time()
                # estimated_latency = baseline.lognorm(quantile, ft, use_sampling)
                # result_lognorm.add(quantile, l, estimated_latency, (time.time() - start) * 1000, m,ft)
                #
                # start = time.time()
                # estimated_latency = baseline.gmm(quantile, ft, use_sampling)
                # result_gmm.add(quantile, l, estimated_latency, (time.time() - start) * 1000, m, ft)
                #
                # baseline.add_actual_latency(l)
    ...
    def compared_baselines(self, quantile):
        # Results
        experimental_results = pd.DataFrame(columns=eval_col_by_keys(['m', 'q', 'l', 'l_pred', 'o', 'msg', 'ft']))
